...
Deq.prototype.pushFront = function (value) {
...
Deq.prototype.pushBack = function (value) {
    if (this.size === this.maxSize) {
        return 'error';
    }
...
Deq.prototype.popBack = function () {
    if (!this.size) {
        return 'error';
    }
...
Deq.prototype.popFront = function () {
    if (!this.size) {
        return 'error';
    }
...
function solve() {
    ...
    commandsArr.forEach(el => {
        ...
        if (commands[0] === 'push_front') {
            const tmp = myDeq.pushFront(Number(commands[1]));
            if (tmp === 'error') result.push(tmp);
        }
        if (commands[0] === 'push_back') {
            const tmp = myDeq.pushBack(Number(commands[1]));
            if (tmp === 'error') result.push(tmp);
        }
        if (commands[0] === 'pop_front') result.push(myDeq.popFront());
        if (commands[0] === 'pop_back') result.push(myDeq.popBack());
    })
