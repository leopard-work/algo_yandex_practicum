...
/*
    ### Принцип работы
        ...
        Для хранения элементов в таблице создан массив определенного размера (по умолчанию 10000)
        Для расчета хеша строки используется функция polyHash (алгоритм вычисления полиномиального хеша) которая принимает аргументы a - основание (по умолчанию 103), m - модуль (по умолчанию 10003) и строку s
        Хеш будет являться индексом в массиве куда попадет элемент
        Для разрешения коллизий используется связный список, элемент сначала попадает в список, затем список записывается в массив по хеш ключу

    ### Доказательство корректности

        В соответствии с заданием реализованы операции "put key", "get key" и "delete key" которые работают в среднем за O(1)

    ### Временная сложность

        В среднем случае скорость работы O(1)
        В худшем случае O(n)
