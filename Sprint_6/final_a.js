...
/*
    ### Принцип работы
        ...
        Для поиска максимального остовного дерева используется алгоритм Прима. В алгоритме используется приоритетная очередь и для оптимизации времени ее работы применяется двоичная куча.

        1. Начиная с произвольной вершины извлекаем ее и добавляем в кучу все ребра со смежными вершинами (если они не были посещены).
        ...
        3. Результатом будет являться сумма всех элементов массива результатов.
        4. Алгоритм будет работать пока все вершины не будет посещены, если такие остались - программа выдаст ошибку.

    ### Доказательство корректности

        Т.к. алоритм всегда извлекает ребро с максимальным весом, максимальное остовное дерево будет корректным.

    ### Временная сложность

        С учетом применения двоичной кучи - O(E * log(V))
