const readline = require("readline");
const fs = require("fs");
const path = require("path");

let curLine = 0;
let n = 0;
let m = 0;
let graph = null;
let weight = new Map();


readline
    .createInterface({
        input: fs.createReadStream(path.join(__dirname, "input.txt"))
    })
    .on("line", line => {
        if (curLine === 0) {
            [n,m] = line.split(/\s/).map(item => parseInt(item));
            graph = new Array(n + 1).fill(null).map(() => []);
        } else {
            const [a,b,c] = line.split(/\s/).map(item => parseInt(item));
            graph[a].push(b);
            graph[b].push(a);
            // weight.set(`${b}-${a}`, c);
            // weight.set(`${a}-${b}`, c);
            if (!weight.has(`${a}-${b}`) || weight.get(`${a}-${b}`) > c) {
                weight.set(`${a}-${b}`, c);
            }
            if (!weight.has(`${b}-${a}`) || weight.get(`${b}-${a}`) > c) {
                weight.set(`${b}-${a}`, c);
            }
        }
        curLine++;
    })
    .on("close", () => ans())

function getMinDistance(dist, visited) {
    let currentMinimum = Number.POSITIVE_INFINITY;
    let currentMinimumVertex = null;

    for (let i = 1; i <= graph.length; i++) {
        if (!visited[i] && dist[i] < currentMinimum) {
            currentMinimum = dist[i];
            currentMinimumVertex = i;
        }
    }

    // for (let v of graph) {
    //     if (!visited[v[0]] && dist[v[0]] < currentMinimum) {
    //         currentMinimum = dist[v[0]];
    //         currentMinimumVertex = v[0];
    //     }
    // }
    return currentMinimumVertex;
}

function relax(dist, u, v) {
    // Проверяем, не получился ли путь короче найденного ранее.
    if (dist[v] > dist[u] + weight.get(`${u}-${v}`)) {
        dist[v] = dist[u] + weight.get(`${u}-${v}`);
    }
}

function landmark(start = 3) {
    const dist = new Array(n + 1).fill(Number.POSITIVE_INFINITY);
    const visited = new Array(n + 1).fill(false);

    dist[start] = 0;

    while (true) {
        let u = getMinDistance(dist, visited);

        if (u === null || dist[u] === Number.POSITIVE_INFINITY) {
            break;
        }

        visited[u] = true;

        for (let i = 0; i < graph[u]?.length; i++) {
            relax(dist, u, graph[u][i])
        }
   }

   dist.shift();
   return dist;
}

function ans() {
    for (let i = 1; i <= n; i++) {
        let ans = landmark(i);
        for (let j = 0; j < ans.length; j++) {
            if (ans[j] === Number.POSITIVE_INFINITY) {
                ans[j] = -1;
            }
        }
        console.log(ans.join(" "))
    }
}